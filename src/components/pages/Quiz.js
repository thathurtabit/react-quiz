...
export default class Quiz extends Component {
  ...
  componentWillMount() {
    // Shuffle Answers
    const shuffledAnswer1Choices = quizData.map(round =>
      this.shuffleArray(round.answer1)
    );
    const shuffledAnswer2Choices = quizData.map(round =>
      this.shuffleArray(round.answer2)
    );
    const shuffledAnswer3Choices = quizData.map(round =>
      this.shuffleArray(round.answer3)
    );
    const shuffledAnswer4Choices = quizData.map(round =>
      this.shuffleArray(round.answer4)
    );
    const shuffledAnswer5Choices = quizData.map(round =>
      this.shuffleArray(round.answer5)
    );
  ...
  checkNextButton() {
    this.setState({
      next: {
        text:
          this.state.round === this.state.roundsTotal ? "See Results" : "Next",
        disabled: false
      }
    });
  }
  ...
  handleAnswerSelected(event) {
    ...
    let currentAnswersArray = [
      ...event.currentTarget.querySelectorAll("input[type=radio]")
    ];
        ...
        <Fade in={this.state.show}>
          <Wrapper>
            <Helmet>
              ...
              <meta
                name="description"
                content={`Learn which design type you are`}
              />
            </Helmet>
            <QuizWrap
              style={{ display: this.state.display.quiz ? "block" : "none" }}
            >
              <QuestionHeader
                question={this.state.question}
                title={this.state.questionTitle}
                intro={this.state.questionIntro}
                showRound={this.state.display.quiz}
